...
QueueManager::QueueManager(PhysicalDevicePtr _physicalDevice, LogicalDevicePtr _device)
  : physicalDevice(_physicalDevice), device(_device) {
    ...
    drv_assert(drv::get_physical_device_queue_families(physicalDevice, &familyCount, nullptr),
    ...
    for (QueueFamily& family : queueFamilies) {
        FamilyInfo& info = families[family.handle];
        info.mask = family.commandTypeMask;
        info.queues.reserve(family.queueCount);
        for (unsigned int i = 0; i < family.queueCount; ++i) {
            QueuePtr queue = drv::get_queue(device, family.handle, i);
            drv_assert(queue != NULL_HANDLE, "Could not get queue");
...
static unsigned int mask_match_count(CommandTypeMask a, CommandTypeMask b) {
    unsigned int ret = 0;
    CommandTypeMask ab = a & b;
    while (ab > 0) {
        if (ab & 1)
            ret++;
        ab >>= 1;
...
unsigned int QueueManager::available(StrHash group, const AcquireInfo& info) const {
