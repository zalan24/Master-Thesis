...
drv::DescriptorSetLayoutPtr drv_cpu::create_descriptor_set_layout(
...
bool drv_cpu::destroy_descriptor_set_layout(drv::LogicalDevicePtr,
...
drv::DescriptorPoolPtr drv_cpu::create_descriptor_pool(drv::LogicalDevicePtr device,
                                                       const drv::DescriptorPoolCreateInfo* info) {
    return drv::NULL_HANDLE;
}

bool drv_cpu::destroy_descriptor_pool(drv::LogicalDevicePtr device, drv::DescriptorPoolPtr pool) {
    return false;
}

bool drv_cpu::allocate_descriptor_sets(drv::LogicalDevicePtr device,
                                       const drv::DescriptorSetAllocateInfo* allocateInfo,
                                       drv::DescriptorSetPtr* sets) {
    return false;
}

bool drv_cpu::update_descriptor_sets(drv::LogicalDevicePtr device, uint32_t descriptorWriteCount,
